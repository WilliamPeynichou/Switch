...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
     ...
     */
    #[ORM\OneToMany(targetEntity: GameSession::class, mappedBy: 'masterUser')]
    private Collection $gameSessions;

    /**
     * @var Collection<int, SessionParticipant>
     */
    #[ORM\OneToMany(targetEntity: SessionParticipant::class, mappedBy: 'user')]
    ...
    public function addGameSession(GameSession $gameSession): static
    {
        if (!$this->gameSessions->contains($gameSession)) {
            ...
            $gameSession->setMasterUser($this);
        }
        return $this;
    }
    ...
    public function removeGameSession(GameSession $gameSession): static
    {
        if ($this->gameSessions->removeElement($gameSession)) {
            if ($gameSession->getMasterUser() === $this) {
                $gameSession->setMasterUser(null);
            }
        }
        return $this;
    }
    ...
    public function addSessionParticipant(SessionParticipant $sessionParticipant): static
    {
        if (!$this->sessionParticipants->contains($sessionParticipant)) {
            ...
            $sessionParticipant->setUser($this);
        }
        return $this;
    }
    ...
    public function removeSessionParticipant(SessionParticipant $sessionParticipant): static
    {
        if ($this->sessionParticipants->removeElement($sessionParticipant)) {
            if ($sessionParticipant->getUser() === $this) {
                $sessionParticipant->setUser(null);
            }
        }
        return $this;
    }
